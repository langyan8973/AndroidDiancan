...
//		System.out.println("mTouchX  " + mTouch.x + "  mTouchY  " + mTouch.y);
	...
	/**
	 * Author : hmg25 Version: 1.0 Description : GradientDrawable
	 */
	...
	public void drawCurrentPageShadow(Canvas canvas) {
		...
		if (mIsRTandLB) {
			...
			rightx = (int) mBezierControl1.x + 15;
			mCurrentPageShadow = mFrontShadowDrawableVLR;
		} else {
			leftx = (int) (mBezierControl1.x - 15);
		...
		{
			float a = (mBezierControl2.y-mTouch.y)/(mBezierControl2.x-mTouch.x);
			float b = (mBezierControl2.x*mTouch.y-mBezierControl2.y*mTouch.x)/(mBezierControl2.x-mTouch.x);
			float cx=(0-b)/a;
			mPath1.lineTo(cx, 0);
		}
		else {			
		...
		if (mIsRTandLB) {
			...
			rightx = (int) (mBezierControl2.y + 15);
			mCurrentPageShadow = mFrontShadowDrawableHTB;
		} else {
			leftx = (int) (mBezierControl2.y - 15);
	...
	private void startAnimation(int delayMillis) {
		...
		if (!toRight) {
			...
			dx = (int) (mWidth - mTouch.x+100);
		}
		if (mCornerY > 0) {
			//为解决小米手机上翻页后变灰的问题必须使dy减去1，也就是不能让翻走的页完全不可见
